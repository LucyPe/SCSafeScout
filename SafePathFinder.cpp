...
void SafePathFinder::update(BWAPI::UnitInterface* unit) {
	map->updateDangerFunctions(unit);
}
...
void SafePathFinder::showGrid() {
	...
	for (unsigned int i = 0; i < nodes.size(); i++) {
		...
		if (nodes[i] != NULL) {
			Broodwar->drawBoxMap(nodes[i]->getX() * WALK_TILE, nodes[i]->getY() * WALK_TILE, nodes[i]->getX() * WALK_TILE + WALK_TILE, nodes[i]->getY() * WALK_TILE + WALK_TILE, BWAPI::Color(255, 255, 255), false);
			for (unsigned int n = 0; n < 8; n++) {
				//if (nodes[i]->getNeighbour(n) == NULL)
				if (nodes[i]->getDangerCost(n) > 0)
					Broodwar->drawCircleMap(nodes[i]->getX() * WALK_TILE + n, nodes[i]->getY() * WALK_TILE + 3, 1, BWAPI::Color(0, 255 - 200 * n, 10 * n), true);
			}
		}
		/*else {
...
void SafePathFinder::drawEnemiesAttackRange() {
	...
	for (auto player = players.begin(); player != players.end(); ++player) {
		...
		for (auto enemy = units.begin(); enemy != units.end(); ++enemy) {
			...
			int weaponRange = weapon.maxRange() + fmax((*enemy)->getType().height(), (*enemy)->getType().width());
			...
			for (int x = ((position.x - weaponRange) / WALK_TILE); x <= ((position.x + weaponRange) / WALK_TILE); x++) {
				for (int y = ((position.y - weaponRange) / WALK_TILE); y <= ((position.y + weaponRange) / WALK_TILE); y++) {
					if ((Utility::distance(position.x, position.y, x * WALK_TILE, y * WALK_TILE)) <= weaponRange) {
						Broodwar->drawBoxMap(x * WALK_TILE, y * WALK_TILE, x * WALK_TILE + WALK_TILE, y * WALK_TILE + WALK_TILE, BWAPI::Color(255, 0, 0), false);
					}
				}
			}
		}
